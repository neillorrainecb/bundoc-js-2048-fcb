...
//used to monitor if the user already won
...
function setGame(){
	...
	//Loops are code to repeat task inside it, until it fulfill its task

	for(let r=0; r < rows; r++){
		for(let c=0; c < columns; c++){

			//This code is to create a tile throug creating div elements
...
function handleSlide(e){
	console.log(e.code); 
	//e.code value represent what key is being pressed in our keyboard
...
function slideleft(){
	...
	for(let r=0; r < rows; r++){
		...
		//Able to update the ids and color of all tiles
		for(let c = 0; c < columns; c++){
...
// for merging, slide up and down will be mainly through column values.
function slideUp(){
	for(let c=0; c < columns; c++){
		...
		for(let r=0; r<rows; r++){
			...
			let num = board[r][c];
			updateTile(tile, num); //update the color of the tile
...
function slideDown(){
	for(let c=0; c < columns; c++){
		...
		let changedIndices = []; // this variabble and loop will record the position of the tiles that have changed/
		for (let r = 0; r < rows; r++) { 
		    if (originalRow[r] !== row[r]){
		        changedIndices.push(r);
		    }
...
function checkWin(){
	for(let r = 0; r < rows; r++){
		for(let c = 0; c < columns; c++){
			...
			else if(board[r][c] == 4096 && is8192Exist == false){
